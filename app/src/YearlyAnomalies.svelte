<script>
    ...
    $: byYear = Array.from(group(dataClean, d => d.date.getFullYear())).map(([key, values]) => ({
    ...
    $: baseTemp = mean(byYear
        .filter(d => d.year >= $contextMinYear && d.year < $contextMaxYear), d => d.tAvg);
        ...
        <g transform="translate(0,{padding.top})">
                ...
                {#each xScale.ticks(8) as tick, i}
                    <g class="tick" transform="translate({xScale(tick)}, 0)">
                        ...
                        <g transform="translate(0,0)">
                            <text
                                y="{height-padding.bottom-padding.top+25}">
                                {format(tick, i)}
                            </text>
                        </g>
                    </g>
            ...
            <g class="values">
                {#each byYear as d}
                    <g
                        class="year"
                        transform="translate({xScale(d.year+0.5)},0)">

                        {#if d.tAvg < baseTemp}
                            <line
                                class="colder"
                                y1={yScale(0)}
                                y2={yScale(d.tAvg - baseTemp)} />
                        {/if}
                        {#if d.tAvg > baseTemp}
                            <line
                                class="hotter"
